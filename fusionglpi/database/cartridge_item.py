...
printer_models = db.Table('cartridge_compatibility',
    db.Column('cartridge_item_id', db.Integer, db.ForeignKey('cartridge_item.id')),
    db.Column('printer_model_id', db.Integer, db.ForeignKey('printer_model.id'))
)
...
class CartridgeItem(db.Model):
    ...
    manufacturer_id = db.Column(db.Integer, nullable=False, server_default="0")
    ...
    cartridge_item_type_id = db.Column(db.Integer, nullable=False, server_default="0")
    ...
    printer_models = db.relationship('PrinterModel', secondary=printer_models,
        backref=db.backref('cartridge_item', lazy='dynamic'))
